...
async function generateIntimVariantOne({
  ...
  if (mentionCandidates.length) {
    ...
    instructions.push(
      'Имея один шанс из трёх, можешь добавить случайного зрителя или зрителей, используя переменные [random_chatter] — бот подставит имена, но добавленные зрители не должны быть основным объектом влечения, либо косвенно упомянуты, либо упомянуты в качестве третьего одновременного партнёра'
    );
  }
  instructions.push(
