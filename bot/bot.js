...
const WHAT_SYSTEM_PROMPT =
  ...
  'Отвечай только одной короткой фразой в нижнем регистре, описывающей предмет, явление, без пояснений и знаков препинания. ' +
  'Меняй формулировки и добавляй абсурдные детали, чтобы каждый ответ звучал свежо и забавно.';
  ...
  if (mentionCandidates.length) {
    instructions.push(
      `Среди зрителей сейчас: ${mentionCandidates
        .map((name) => `@${name}`)
        .join(', ')}. Можешь упомянуть одного из них или использовать переменные [random_chatter] для случайных зрителей.`
    );
  } else {
    instructions.push(
      'Если хочешь добавить случайного зрителя, используй переменные [random_chatter] — бот подставит имена.'
    );
  }
  instructions.push(
